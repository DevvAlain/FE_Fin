...
const EmailVerification: React.FC<EmailVerificationProps> = ({ token, returnUrl }) => {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        const result = await authService.verifyEmail(token, returnUrl || undefined);
  ...
  const handleResendVerification = async () => {
    ...
    try {
      const result = await authService.resendVerificationEmail(email, window.location.origin);
